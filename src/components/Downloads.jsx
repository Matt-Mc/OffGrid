...
export function Downloads({settings,queue,storage,online,onManage,onPlay,videos,refreshQueue,composerRequest,captures = {items:[]},onAcknowledgeCapture}) {
  ...
  return <>
    <header className="page-header"><div><h1>Downloads</h1></div>{queue.jobs.length > 0 && <button className="secondary-button" disabled={queueBusy} onClick={toggleQueue}>{queue.paused ? 'Resume queue' : 'Stop after current'}</button>}</header>
