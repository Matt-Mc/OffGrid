...
contextBridge.exposeInMainWorld("offgrid", {
  ...
  downloadJellyfin:(id,options)=>ipcRenderer.invoke('jellyfin:download',id,options),
  ...
  downloadPlex:(id,options)=>ipcRenderer.invoke('plex:download',id,options),
  previewServerSeason:(provider,id,requestId)=>ipcRenderer.invoke('servers:preview-season',provider,id,requestId),
  downloadServerBatch:(provider,args)=>ipcRenderer.invoke('servers:add-batch',provider,args),
  serverSubtitleTracks:(provider,id)=>ipcRenderer.invoke('servers:subtitles',provider,id),
  ...
  checkAppUpdate:(automatic=false)=>ipcRenderer.invoke('app:update-check',automatic),
  downloadAppUpdate:()=>ipcRenderer.invoke('app:update-download'),
  cancelAppUpdate:()=>ipcRenderer.invoke('app:update-cancel'),
  onAppUpdate:callback=>listen('app:update-status',callback),
  ...
  pauseDownload:id=>ipcRenderer.invoke('downloads:pause-item',id),
  resumeDownload:(id,options={})=>ipcRenderer.invoke('downloads:resume',id,options),
  previewDownloads:args=>ipcRenderer.invoke('downloads:preview',args),
  cancelPreview:requestId=>ipcRenderer.invoke('downloads:cancel-preview',requestId),
  addDownloadBatch:args=>ipcRenderer.invoke('downloads:add-batch',args),
  retrySubtitles:id=>ipcRenderer.invoke('library:retry-subtitles',id),
  listCaptures:()=>ipcRenderer.invoke('capture:list'),
  acknowledgeCapture:id=>ipcRenderer.invoke('capture:acknowledge',id),
  onCapturesUpdate:callback=>listen('capture:update',callback),
  subtitleUrl:(videoId,assetId)=>`media://subtitle/${encodeURIComponent(videoId)}/${encodeURIComponent(assetId)}`,
  ...
  onSettingsOpen:callback=>listen("settings:open",callback),
	...
	startDownload: (url, quality, options={}) =>
		ipcRenderer.invoke("download:start", { ...options,url,quality }),
